...
mcontrol <- function(y, grp.id, treat.id, group.size=3, Gamma=4, GammaInc=1)
		{
...
#Calculate P-values and Store
  ...
  out <- cbind(gamma.seq, pvals)
  colnames(out) <- c("Gamma", "L. Bound P-Value", "U. Bound P-Value")
  cat("Rosenbaum Sensitivity Test for Wilcoxon Strat. Rank P-Value \n")
  print(out)
  cat("\n")
